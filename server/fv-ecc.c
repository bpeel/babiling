...
static void
make_pub_key_in_point(struct fv_ecc *ecc,
...
fv_ecc_new(void)
{
        struct fv_ecc *ecc = fv_alloc(sizeof *ecc);
        ...
        if (ecc->bn_ctx == NULL)
                fv_fatal("Error creating BN_CTX");

        ecc->group = EC_GROUP_new_by_curve_name(NID_secp256k1);

        if (ecc->group == NULL)
                fv_fatal("Error creating EC_GROUP");
...
{
        uint8_t private_key[FV_ECC_PRIVATE_KEY_SIZE];
        int result;

        result = RAND_bytes(private_key, FV_ECC_PRIVATE_KEY_SIZE);
        assert(result);

        return fv_ecc_create_key(ecc, private_key);
}
...
static void
encode_bignumber(struct fv_ecc *ecc,
                 BIGNUM *bn,
                 struct fv_buffer *data_out)
{
        ...
        fv_proto_add_16(data_out, num_bytes);

        fv_buffer_ensure_size(data_out, data_out->length + num_bytes);
        BN_bn2bin(bn, data_out->data + data_out->length);
        data_out->length += num_bytes;
}

static void
encode_pubkey(struct fv_ecc *ecc,
              ...
              struct fv_buffer *data_out)
{
        ...
        /* Add the curve type */
        fv_proto_add_16(data_out, EC_GROUP_get_curve_name(ecc->group));
                                 ...
                                 struct fv_buffer *data_out)
{
        ...
        ephemeral_key = fv_ecc_create_random_key(ecc);
        ...
        fv_buffer_set_length(data_out, iv_offset + iv_length);
                       ...
                       struct fv_buffer *data_out)
{
        ...
        fv_buffer_ensure_size(data_out,
...
fv_ecc_encrypt_with_point(struct fv_ecc *ecc,
                           ...
                           struct fv_buffer *data_out)
{
        fv_ecc_encrypt_with_point_begin(ecc, public_key, data_out);
        fv_ecc_encrypt_update(ecc, data_in, data_in_length, data_out);
        fv_ecc_encrypt_end(ecc, data_out);
}

static bool
decode_big_number(struct fv_ecc *ecc,
                  const uint8_t **data_in,
                  size_t *data_in_length,
                  BIGNUM *bn)
{
        ...
        num_bytes = fv_proto_get_16(*data_in);
...
static bool
decode_pub_key(struct fv_ecc *ecc,
               const uint8_t **data_in,
               size_t *data_in_length,
               EC_POINT *public_key)
{
        ...
        if (fv_proto_get_16(*data_in) != EC_GROUP_get_curve_name(ecc->group))
                return false;
