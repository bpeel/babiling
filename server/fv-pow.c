...
#define FV_POW_NONCES_PER_CHECK 4096

static void
unref_cookie(struct fv_pow_cookie *cookie)
{
...
static void
remove_queued_cookie(struct fv_pow_cookie *cookie)
{
        /* This should only be called with the lock */
...
{
        struct fv_pow_cookie *cookie, *tmp;
        int i;
        ...
        fv_list_for_each_safe(cookie, tmp, &pow->queue, link)
...
idle_cb(struct fv_main_context_source *source,
        void *user_data)
{
        struct fv_pow_cookie *cookie = user_data;
        struct fv_pow *pow = cookie->pow;
        fv_pow_calculate_func func;
        void *cb_user_data;
        uint64_t nonce;
...
static void
set_solution(struct fv_pow_cookie *cookie,
             uint64_t nonce)
{
        struct fv_pow *pow = cookie->pow;
        ...
        if (!cookie->done) {
                ...
                cookie->done = true;
                cookie->idle_source =
                        fv_main_context_add_idle(NULL, idle_cb, cookie);
...
static void
find_nonce(struct fv_pow_cookie *cookie,
           int nonce_offset)
{
        struct fv_pow *pow = cookie->pow;
        ...
        while (!cookie->done && !pow->quit) {
                ...
                for (i = 0; i < FV_POW_NONCES_PER_CHECK; i++) {
                        nonce_be = FV_UINT64_TO_BE(nonce);
                        ...
                        pow_value = FV_UINT64_FROM_BE(pow_value);
...
{
        struct fv_pow *pow = user_data;
        struct fv_pow_cookie *cookie;
        int nonce_offset;

        pthread_mutex_lock(&pow->mutex);

        while (true) {
                while (!pow->quit && fv_list_empty(&pow->queue))
                        pthread_cond_wait(&pow->cond, &pow->mutex);

                if (pow->quit)
                        break;

                cookie = fv_container_of(pow->queue.next,
                                          struct fv_pow_cookie,
                                          link);
...
fv_pow_new(void)
{
        ...
        pow = fv_alloc(sizeof *pow);

        pow->n_threads = fv_cpus_count();

        pthread_mutex_init(&pow->mutex, NULL);
        pthread_cond_init(&pow->cond, NULL);

        fv_list_init(&pow->queue);
        pow->quit = false;

        pow->threads = fv_alloc(sizeof (pthread_t) * pow->n_threads);

        for (i = 0; i < pow->n_threads; i++)
                pow->threads[i] = fv_create_thread(thread_func, pow);
                  ...
                  void *user_data)
{
        ...
        target = fv_pow_calculate_target(length + sizeof (uint64_t),
                                          pow_per_byte,
                                          pow_extra_bytes);
        ...
        fv_list_insert(pow->queue.next, &cookie->link);
                        ...
                        size_t length)
{
        ...
        return FV_UINT64_FROM_BE(pow_value);
}
              ...
              int pow_extra_bytes)
{
        ...
        pow_value = fv_pow_calculate_value(payload, length);

        target = fv_pow_calculate_target(length,
                                          pow_per_byte,
                                          pow_extra_bytes);
