...
void
fv_address_encode(const struct fv_address *address,
                   char *output)
{
        ...
        *(p++) = address->stream;
        ...
        memcpy(output, "BM-", 3);
        address_length = fv_base58_encode(bin_address,
                                           p - bin_address,
                                           output + 3);
...
bool
fv_address_decode(struct fv_address *address,
                   const char *address_string)
{
        ...
        bin_address_length = fv_base58_decode(address_string,
...
bool
fv_address_equal(const struct fv_address *a,
                  const struct fv_address *b)
{
...
void
fv_address_from_network_keys(struct fv_address *address,
                              uint8_t version,
                              uint8_t stream,
...
void
fv_address_get_tag(const struct fv_address *address,
                    uint8_t *tag,
                    uint8_t *tag_private_key)
{
        ...
        fv_proto_add_var_int(&buffer, address->version);
        fv_proto_add_var_int(&buffer, address->stream);
        ...
        if (tag) {
                memcpy(tag,
                       hash2 + FV_ECC_PRIVATE_KEY_SIZE,
                       FV_ADDRESS_TAG_SIZE);
        }
        if (tag_private_key) {
                memcpy(tag_private_key, hash2, FV_ECC_PRIVATE_KEY_SIZE);
        }
}
