...
struct fv_crypto {
        ...
        struct fv_slice_allocator cookie_allocator;
...
enum fv_crypto_cookie_type {
        FV_CRYPTO_COOKIE_CREATE_KEY,
        FV_CRYPTO_COOKIE_CREATE_PUBKEY_BLOB,
        FV_CRYPTO_COOKIE_CHECK_PUBKEY,
        FV_CRYPTO_COOKIE_CREATE_MSG_BLOB,
        FV_CRYPTO_COOKIE_CREATE_PUBLIC_KEY,
        FV_CRYPTO_COOKIE_DECRYPT_MSG,
        FV_CRYPTO_COOKIE_GENERATE_ACKDATA
};

struct fv_crypto_cookie {
        int ref_count;

        enum fv_crypto_cookie_type type;
        ...
        union {
                ...
                struct {
                        ...
                        struct fv_blob *result;
                        int64_t timestamp;
                } create_msg_blob;

                struct {
                        struct fv_key_params params;
                        uint8_t signing_key[FV_ECC_PUBLIC_KEY_SIZE];
                        uint8_t encryption_key[FV_ECC_PUBLIC_KEY_SIZE];
                        struct fv_key *key;
                } create_public_key;
...
static struct fv_crypto_cookie *
new_cookie(struct fv_crypto *crypto,
           enum fv_crypto_cookie_type type,
           void *func,
           void *user_data)
{
        struct fv_crypto_cookie *cookie;

        /* This should only be called with the lock */

        cookie = fv_slice_alloc(&crypto->cookie_allocator);
        ...
        fv_list_insert(crypto->queue.prev, &cookie->link);
...
static void
free_create_key_cookie(struct fv_crypto_cookie *cookie)
{
        if (cookie->create_key.key)
                fv_key_unref(cookie->create_key.key);
        if ((cookie->create_key.params.flags & FV_KEY_PARAM_LABEL))
                fv_free((char *) cookie->create_key.params.label);
}

static void
free_create_pubkey_blob_cookie(struct fv_crypto_cookie *cookie)
{
        if (cookie->create_pubkey_blob.key)
                fv_key_unref(cookie->create_pubkey_blob.key);
        if (cookie->create_pubkey_blob.blob)
                fv_blob_unref(cookie->create_pubkey_blob.blob);
}

static void
free_check_pubkey_cookie(struct fv_crypto_cookie *cookie)
{
        if (cookie->check_pubkey.key)
                fv_key_unref(cookie->check_pubkey.key);
        if (cookie->check_pubkey.blob)
                fv_blob_unref(cookie->check_pubkey.blob);
}

static void
free_create_msg_blob_cookie(struct fv_crypto_cookie *cookie)
{
        if (cookie->create_msg_blob.from_key)
                fv_key_unref(cookie->create_msg_blob.from_key);
        if (cookie->create_msg_blob.to_key)
                fv_key_unref(cookie->create_msg_blob.to_key);
        if (cookie->create_msg_blob.content)
                fv_blob_unref(cookie->create_msg_blob.content);
        if (cookie->create_msg_blob.result)
                fv_blob_unref(cookie->create_msg_blob.result);
}

static void
free_create_public_key_cookie(struct fv_crypto_cookie *cookie)
{
        if (cookie->create_public_key.key)
                fv_key_unref(cookie->create_public_key.key);
        if ((cookie->create_public_key.params.flags & FV_KEY_PARAM_LABEL))
                fv_free((char *) cookie->create_public_key.params.label);
}

static void
free_decrypt_msg_cookie(struct fv_crypto_cookie *cookie)
{
        int i;

        for (i = 0; i < cookie->decrypt_msg.n_keys; i++)
                fv_key_unref(cookie->decrypt_msg.keys[i]);
        fv_free(cookie->decrypt_msg.keys);

        if (cookie->decrypt_msg.result)
                fv_blob_unref(cookie->decrypt_msg.result);

        if (cookie->decrypt_msg.chosen_key)
                fv_key_unref(cookie->decrypt_msg.chosen_key);

        fv_blob_unref(cookie->decrypt_msg.blob);
}

static void
free_generate_ackdata_cookie(struct fv_crypto_cookie *cookie)
{
}

static void
unref_cookie(struct fv_crypto_cookie *cookie)
{
        /* This should only be called with the lock */

        if (--cookie->ref_count <= 0) {
                switch (cookie->type) {
                case FV_CRYPTO_COOKIE_CREATE_KEY:
                        free_create_key_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_CREATE_PUBKEY_BLOB:
                        free_create_pubkey_blob_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_CHECK_PUBKEY:
                        free_check_pubkey_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_CREATE_MSG_BLOB:
                        free_create_msg_blob_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_CREATE_PUBLIC_KEY:
                        free_create_public_key_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_DECRYPT_MSG:
                        free_decrypt_msg_cookie(cookie);
                        break;
                case FV_CRYPTO_COOKIE_GENERATE_ACKDATA:
                        free_generate_ackdata_cookie(cookie);
                        break;
                }

                fv_slice_free(&cookie->crypto->cookie_allocator, cookie);
        }
}

static void
create_key(struct fv_crypto *crypto,
           uint8_t *private_key,
           uint8_t *public_key)
{
        int result;

        result = RAND_bytes(private_key, FV_ECC_PRIVATE_KEY_SIZE);
        assert(result);

        fv_ecc_make_pub_key_bin(crypto->ecc,
                                 private_key,
                                 public_key);
}
...
static void
handle_create_key(struct fv_crypto_cookie *cookie)
{
        struct fv_crypto *crypto = cookie->crypto;
        uint8_t private_signing_key[FV_ECC_PRIVATE_KEY_SIZE];
        uint8_t pub_signing_key[FV_ECC_PUBLIC_KEY_SIZE];
        uint8_t private_encryption_key[FV_ECC_PRIVATE_KEY_SIZE];
        uint8_t pub_encryption_key[FV_ECC_PUBLIC_KEY_SIZE];
        uint8_t sha_hash[SHA512_DIGEST_LENGTH];
        uint8_t ripe[RIPEMD160_DIGEST_LENGTH];
        char address_string[FV_ADDRESS_MAX_LENGTH + 1];
        SHA512_CTX sha_ctx;
        int attempts = 0;

        fv_log("Generating key pair");
        ...
        cookie->create_key.params.flags |= FV_KEY_PARAM_RIPE;
        cookie->create_key.params.ripe = ripe;

        cookie->create_key.params.flags |= FV_KEY_PARAM_PRIVATE_KEYS;
        cookie->create_key.params.private_signing_key = private_signing_key;
        cookie->create_key.params.private_encryption_key =
                private_encryption_key;

        cookie->create_key.params.flags |= FV_KEY_PARAM_PUBLIC_KEYS;
        ...
        cookie->create_key.key = fv_key_new(crypto->ecc,
                                             &cookie->create_key.params);

        fv_address_encode(&cookie->create_key.key->address, address_string);

        fv_log("Key pair generated after %i attempt%s. Address is %s",
                attempts,
                attempts == 1 ? "" : "s",
                address_string);
}
...
static void
append_key_base(struct fv_key *key,
                ...
                size_t *behaviors_offset)
{
        ...
        fv_proto_add_64(buffer, key->last_pubkey_send_time);
        fv_proto_add_var_int(buffer, key->address.version);
        fv_proto_add_var_int(buffer, key->address.stream);

        if (behaviors_offset)
                *behaviors_offset = buffer->length;

        fv_proto_add_32(buffer, FV_PROTO_PUBKEY_BEHAVIORS);
        fv_proto_add_public_key(buffer, key->signing_key);
        fv_proto_add_public_key(buffer, key->encryption_key);
}

static void
append_signature(struct fv_buffer *buffer,
                 struct fv_key *key,
                 const uint8_t *data,
                 size_t length)
{
        ...
        fv_proto_add_var_int(buffer, sig_length);
        fv_buffer_append(buffer, sig, sig_length);
}

static void
append_v34_key_base(struct fv_key *key,
                    struct fv_buffer *buffer,
                    size_t *behaviors_offset)
{
        append_key_base(key, buffer, behaviors_offset);

        fv_proto_add_var_int(buffer, key->pow_per_byte);
        fv_proto_add_var_int(buffer, key->pow_extra_bytes);
...
create_v4_key(struct fv_crypto *crypto,
              struct fv_key *key)
{
        struct fv_blob *blob;
        struct fv_buffer buffer;
        ...
        struct fv_buffer encrypted_buffer;

        fv_buffer_init(&buffer);
        fv_blob_dynamic_init(&encrypted_buffer,
                              FV_PROTO_INV_TYPE_PUBKEY);

        append_v34_key_base(key, &buffer, &behaviors_offset);

        tag_public_key_point =
                fv_ecc_make_pub_key_point(crypto->ecc,
                                           key->tag_private_key);

        fv_buffer_append(&encrypted_buffer,
                          buffer.data,
                          behaviors_offset);
        fv_buffer_append(&encrypted_buffer,
                          key->tag,
        ...
        fv_ecc_encrypt_with_point(crypto->ecc,
                                   tag_public_key_point,
        ...
        blob = fv_blob_dynamic_end(&encrypted_buffer);
...
static struct fv_blob *
create_v3_key(struct fv_key *key)
{
        struct fv_buffer buffer;

        fv_blob_dynamic_init(&buffer, FV_PROTO_INV_TYPE_PUBKEY);

        append_v34_key_base(key, &buffer, NULL);

        return fv_blob_dynamic_end(&buffer);
}

static struct fv_blob *
create_v2_key(struct fv_key *key)
{
        struct fv_buffer buffer;
...
static void
handle_create_pubkey_blob(struct fv_crypto_cookie *cookie)
{
        struct fv_key *key = cookie->create_pubkey_blob.key;
...
static bool
check_signature_for_data(struct fv_crypto *crypto,
                         ...
                         size_t signature_length)
{
        ...
        public_key[0] = 0x04;
        memcpy(public_key + 1, network_public_key, FV_ECC_PUBLIC_KEY_SIZE - 1);

        key = fv_ecc_create_key_with_public(crypto->ecc,
                                             NULL, /* private key */
                                             public_key);
...
static void
handle_check_unencrypted_pubkey(struct fv_crypto_cookie *cookie,
                                const struct fv_proto_pubkey *pubkey)
{
        ...
        if (pubkey->signature &&
            !check_signature_for_data(crypto,
                ...
                fv_log("The signature in the pubkey message is invalid");
                return;
        }
        ...
        if (!fv_address_equal(&address, &cookie->check_pubkey.address)) {
                fv_log("The keys in the decrypted pubkey msg do not match "
                        "the key's tag.");
                return;
        }
        ...
        params.flags = (FV_KEY_PARAM_VERSION |
                        FV_KEY_PARAM_STREAM |
        ...
        cookie->check_pubkey.key = fv_key_new(crypto->ecc, &params);
}

static void
handle_check_encrypted_pubkey(struct fv_crypto_cookie *cookie,
                              const struct fv_proto_pubkey *pubkey)
{
        struct fv_crypto *crypto = cookie->crypto;
        ...
        struct fv_buffer buffer;
        struct fv_proto_pubkey decrypted_pubkey;
        EC_KEY *key;

        fv_address_get_tag(&cookie->check_pubkey.address,
                            tag,
                            tag_private_key);

        if (memcmp(tag, pubkey->tag, FV_ADDRESS_TAG_SIZE))
                return;

        key = fv_ecc_create_key(crypto->ecc, tag_private_key);
        ...
        if (fv_ecc_decrypt(crypto->ecc,
                            key,
                            pubkey->encrypted_data,
                            pubkey->encrypted_data_length,
                            &buffer) &&
            fv_proto_get_pubkey(true, /* decrypted */
                                 buffer.data,
                                 buffer.length,
                                 &decrypted_pubkey)) {
...
static void
handle_check_pubkey(struct fv_crypto_cookie *cookie)
{
        struct fv_proto_pubkey pubkey;
        struct fv_blob *blob = cookie->check_pubkey.blob;

        if (blob->type != FV_PROTO_INV_TYPE_PUBKEY)
                return;
...
static void
handle_create_msg_blob(struct fv_crypto_cookie *cookie)
{
        struct fv_crypto *crypto = cookie->crypto;
        struct fv_key *from_key = cookie->create_msg_blob.from_key;
        struct fv_key *to_key = cookie->create_msg_blob.to_key;
        struct fv_blob *content = cookie->create_msg_blob.content;
        const EC_POINT *encryption_key;
        struct fv_buffer signature;
        struct fv_buffer buf;

        fv_blob_dynamic_init(&buf, FV_PROTO_INV_TYPE_MSG);

        /* Leave space for the nonce. The caller will have to
         * calculate this */
        fv_buffer_set_length(&buf, buf.length + sizeof (uint64_t));

        fv_proto_add_64(&buf, cookie->create_msg_blob.timestamp);
        fv_proto_add_var_int(&buf, to_key->address.stream);

        encryption_key = EC_KEY_get0_public_key(to_key->encryption_key);

        fv_ecc_encrypt_with_point_begin(crypto->ecc,
                                         encryption_key,
                                         &buf);
        fv_ecc_encrypt_update(crypto->ecc,
                               content->data,
                               content->size,
                               &buf);

        fv_buffer_init(&signature);
        ...
        fv_buffer_destroy(&signature);

        fv_ecc_encrypt_end(crypto->ecc, &buf);

        cookie->create_msg_blob.result = fv_blob_dynamic_end(&buf);
}

static void
handle_create_public_key(struct fv_crypto_cookie *cookie)
{
        struct fv_crypto *crypto = cookie->crypto;
        struct fv_key *key;

        key = fv_key_new(crypto->ecc,
...
static void
check_signature_in_decrypted_msg(struct fv_crypto_cookie *cookie)
{
        struct fv_proto_decrypted_msg msg;

        fv_log("Successfully decrypted a message using the key “%s”",
                cookie->decrypt_msg.chosen_key->label);

        if (!fv_proto_get_decrypted_msg(cookie->decrypt_msg.result->data,
                                         cookie->decrypt_msg.result->size,
                                         &msg)) {
                fv_log("The decrypted message is invalid");
                goto invalid;
        }
        ...
        if (!check_signature_for_data(cookie->crypto,
                                      ...
                                      msg.sig,
                                      msg.sig_length)) {
                fv_log("The signature in the decrypted message is invalid");
                goto invalid;
        }
...
static void
handle_decrypt_msg(struct fv_crypto_cookie *cookie)
{
        struct fv_crypto *crypto = cookie->crypto;
        struct fv_blob *blob = cookie->decrypt_msg.blob;
        struct fv_buffer buffer;
        struct fv_key *key;
        ssize_t header_size;
        uint64_t pow_nonce;
        ...
        header_size = fv_proto_get_command(blob->data,
                                            blob->size,
        ...
        pow_value = fv_pow_calculate_value(blob->data, blob->size);
        ...
        for (i = 0; i < cookie->decrypt_msg.n_keys; i++) {
                ...
                target = fv_pow_calculate_target(data_length,
                                                  key->pow_per_byte,
                                                  key->pow_extra_bytes);
                ...
                if (fv_ecc_decrypt(crypto->ecc,
                                    ...
                                    data_length,
                                    &buffer)) {
                        cookie->decrypt_msg.chosen_key = fv_key_ref(key);
                        cookie->decrypt_msg.result =
                                fv_blob_dynamic_end(&buffer);
...
static void
handle_generate_ackdata(struct fv_crypto_cookie *cookie)
{
        int result;
...
static void
idle_cb(struct fv_main_context_source *source,
        void *user_data)
{
        struct fv_crypto_cookie *cookie = user_data;
        struct fv_crypto *crypto = cookie->crypto;
        fv_crypto_create_key_func create_key_func;
        fv_crypto_create_pubkey_blob_func create_pubkey_blob_func;
        fv_crypto_create_msg_blob_func create_msg_blob_func;
        fv_crypto_decrypt_msg_func decrypt_msg_func;
        fv_crypto_generate_ackdata_func generate_ackdata_func;
...
{
        struct fv_crypto *crypto = user_data;
        struct fv_crypto_cookie *cookie;
        enum fv_crypto_cookie_type type;
...
fv_crypto_new(void)
{
        struct fv_crypto *crypto = fv_alloc(sizeof *crypto);

        crypto->quit = false;

        fv_list_init(&crypto->queue);

        fv_slice_allocator_init(&crypto->cookie_allocator,
                                 sizeof (struct fv_crypto_cookie),
                                 FV_ALIGNOF (struct fv_crypto_cookie));

        pthread_cond_init(&crypto->cond, NULL);
        pthread_mutex_init(&crypto->mutex, NULL);
        crypto->thread = fv_create_thread(thread_func, crypto);

        crypto->ecc = fv_ecc_new();
                      ...
                      void *user_data)
{
        ...
        if ((params->flags & FV_KEY_PARAM_LABEL))
                cookie->create_key.params.label = fv_strdup(params->label);
                              ...
                              void *user_data)
{
        ...
        cookie = new_cookie(crypto,
                            ...
                            callback,
                            user_data);
        cookie->create_pubkey_blob.key = fv_key_ref(key);
                           ...
                           void *user_data)
{
        ...
        cookie->create_msg_blob.from_key = fv_key_ref(from_key);
        cookie->create_msg_blob.to_key = fv_key_ref(to_key);
        cookie->create_msg_blob.content = fv_blob_ref(content);
                             ...
                             void *user_data)
{
        ...
        assert((params->flags & (FV_KEY_PARAM_PUBLIC_KEYS |
                        ...
                        void *user_data)
{
        ...
        cookie->check_pubkey.blob = fv_blob_ref(blob);
                       ...
                       void *user_data)
{
        ...
        cookie = new_cookie(crypto,
                            ...
                            callback,
                            user_data);
        cookie->decrypt_msg.blob = fv_blob_ref(blob);
        cookie->decrypt_msg.keys =
                fv_memdup(keys, n_keys * sizeof (struct fv_key *));
...
fv_crypto_free(struct fv_crypto *crypto)
{
        ...
        fv_slice_allocator_destroy(&crypto->cookie_allocator);

        fv_ecc_free(crypto->ecc);
