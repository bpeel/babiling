...
{
        if (state->error)
                fv_error_free(state->error);
        fv_buffer_destroy(&state->buf);
}
                   ...
                   ...)
{
        ...
        va_start(ap, format);
        fv_set_error_va_list(&state->error, domain, code, format, ap);
        va_end(ap);
}
                ...
                size_t data_length)
{
        ...
        fv_buffer_append(state->response_buf, data + 4, data_length - 4);
...
read_commands(struct response_state *state,
              int sock)
{
        enum fv_file_error file_error;
        ssize_t got;

        while (true) {
                fv_buffer_ensure_size(&state->buf, state->buf.length + 512);
                ...
                if (got == -1) {
                        set_response_error(state,
                                           &fv_file_error,
                                           fv_file_error_from_errno(errno),
                                           "Error reading from IPC socket: %s",
                                           strerror(errno));
                        break;
                } else if (got == 0) {
                        if (state->buf.length > 0) {
                                file_error = fv_file_error_from_errno(errno);
                                set_response_error(state,
                                                   &fv_file_error,
                                                   file_error,
                            ...
                            uint32_t request_id)
{
        ...
        if (response_state.error) {
                fv_error_propagate(error, response_state.error);
