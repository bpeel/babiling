...
{
        static const struct fv_slab_allocator init = FV_SLAB_STATIC_INIT;

        *allocator = init;
}

static size_t
fv_slab_align(size_t base, int alignment)
{
                  ...
                  size_t size, int alignment)
{
        ...
        offset = fv_slab_align(allocator->slab_used, alignment);

        if (size + offset > FV_SLAB_SIZE) {
                /* Start a new slab */
                slab = fv_alloc(FV_SLAB_SIZE);
                slab->next = allocator->slabs;
                allocator->slabs = slab;

                offset = fv_slab_align(sizeof(struct fv_slab), alignment);
        } else {
