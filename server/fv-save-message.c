                    ...
                    FILE *out)
{
        ...
        while (true) {
                ...
                encoded_size = fv_base64_encode(subject, chunk_size, buf);
...
classify_label(struct fv_key *key)
{
...
static void
write_address(struct fv_key *key,
              const char *address,
              FILE *out)
{
                 ...
                 FILE *out)
{
        ...
        fv_address_encode(&to_key->address, to_address);
        ...
        fv_proto_get_decrypted_msg(blob->data, blob->size, &msg);
