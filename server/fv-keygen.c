...
usage(void)
{
        printf("notbit-keygen - Version " PACKAGE_VERSION "\n"
                  ...
                  struct fv_error **error)
{
        ...
        while ((opt = getopt(argc, argv, options)) != -1) {
                ...
                case 'd':
                        option_pow_per_byte =
                                atoi(optarg) * FV_PROTO_MIN_POW_PER_BYTE;
                        break;

                case 'D':
                        option_pow_extra_bytes =
                                atoi(optarg) * FV_PROTO_MIN_POW_EXTRA_BYTES;
                        break;
                    ...
                    struct fv_error **error)
{
        ...
        fv_ipc_proto_begin_command(&buf, "keygen", 0 /* request_id */);
        fv_proto_add_var_int(&buf, option_version);
        fv_proto_add_var_int(&buf, option_stream);
        fv_proto_add_var_int(&buf, option_pow_per_byte);
        fv_proto_add_var_int(&buf, option_pow_extra_bytes);
        fv_proto_add_8(&buf, option_zeroes);
        fv_proto_add_var_str(&buf, option_label);
        fv_ipc_proto_end_command(&buf, 0 /* command_start */);

        res = fv_ipc_client_send_command(sock,
                                          buf.data,
                                          buf.length,
                                          NULL,
                                          0, /* n_fds */
...
{
        struct fv_proto_var_str address_str;
        uint64_t version;
        uint64_t stream;
        ssize_t header_size;
        ...
        if (!fv_proto_get_var_str(&data, &data_length, &address_str))
                goto error;
...
fv_keygen(int argc, char **argv)
{
        ...
        sock = fv_ipc_client_connect(&error);
        ...
        if (send_keygen_command(sock, &error)) {
                fv_buffer_init(&response_buf);
                if (fv_ipc_client_get_response(sock,
                                                &response_buf,
                                                &error,
                                                0 /* request_id */)) {
                ...
                fv_buffer_destroy(&response_buf);
        } else {
                fprintf(stderr, "%s\n", error->message);
                fv_error_free(error);
                ret = EXIT_FAILURE;
